...
<!-- Index Product ============================================================ -->
      <div class="container">
        <?php if (isset($_GET['search'])) { 
          $object = $_GET['search']?>
          <h3>You searched for '<?php echo $object; ?>'</h3>
      <?php } ?>
      ...
      <?php
          ...
          if($sql){
              while($obj = $sql->fetch_object()) { 
                if ( preg_replace('/\s+/','',$obj->product_name) == preg_replace('/\s+/', '',$object)) {
                    echo '<div class="item">
              ...
              echo '<div class="col-lg-4"><img src="assets/img/product/'.$obj->product_img_name.'"></div>
                    ...
                    if($obj->qty > 0){
                        echo '<p><a href="update-cart.php?action=add&id='.$obj->id.'"><input type="submit" class="add-to-cart" value="Add To Cart"></a>';
                        echo '</div></div>';
                    }
                    else {
                        echo 'Out Of Stock!';
